...
import static org.openmuc.j60870.TypeId.C_IC_NA_1;
import static org.openmuc.j60870.TypeId.C_SC_NA_1;
import static org.openmuc.j60870.TypeId.C_SE_NB_1;
import static org.openmuc.j60870.TypeId.M_ME_NB_1;
 ...
 * @author will
 */
public class OneBPumpControllerConnectionManager 
extends ControllerConnectionManager
{
    ...
    public OneBPumpControllerConnectionManager(IntCliParameter commonAddrParam_)
    {
	super("Pump",commonAddrParam_);
        text_latest_order_sent= new JLabel("Latest_order_sent:");
        value_latest_order_sent= new JLabel(state.toString());
	 ...
	 try {
		 ...
		 case M_ME_NB_1:			
		    ...
		    state=BINARY_PUMP_STATE.getStateFromVal((short)scaledValue.getUnnormalizedValue());
                    printWithName("Received pump states: " + state.toString());
                    value_latest_received_pumpstate.setText(state.toString());
		    break;
...
//SENDING
	...
	if(connection==null)
	    return;
        state=BINARY_PUMP_STATE.FORWARD;
        try {
		sendPumpStateCommand(state);
	} catch (IOException e1) 
	{
    ...
    protected  void sendPumpStateCommand(BINARY_PUMP_STATE state) throws IOException
    {
	if(connection==null)
	    return;
        locked_logger.log(state.toString());
	short command=state.getNumVal();
	connection.setScaledValueCommand(commonAddrParam.getValue(), 
        ...
        printNewLine();
	printWithName("** Sending Motor command: "+state.toString()+" **\n");
        value_latest_order_sent.setText(state.toString());
    }
...
//INTERFACE HANDLING
	...
	//"FORWARD" Button
    	JButton f_all = new JButton("Forward");
